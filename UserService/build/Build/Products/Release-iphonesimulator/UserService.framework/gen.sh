...
DEVICE_DIR=${WRK_DIR}/Build/Intermediates.noindex/${FMK_NAME}.build/BuildProductsPath/Release-iphoneos/${FMK_NAME}.framework
SIMULATOR_DIR=${WRK_DIR}/Build/Products/Release-iphonesimulator/${FMK_NAME}.framework
xcodebuild -project '../TestFramework.xcodeproj' -scheme "${FMK_NAME}" -configuration Release -sdk iphoneos -derivedDataPath 'build' clean archive
xcodebuild -project '../TestFramework.xcodeproj' -scheme "${FMK_NAME}" -configuration Release -sdk iphonesimulator clean build -derivedDataPath 'build'
# Cleaning the oldest.
if [ -d "${INSTALL_DIR}" ]
then
rm -rf "${INSTALL_DIR}"
